...
export default function Header() {
  ...
  const { isAdmin, ready: adminReady, logout: adminLogout } = useAdmin();
  const {
    nickname,
    authenticated,
    hasProfile,
  ...
  const ready = adminReady && authReady;
  const showNav = authReady && authenticated && hasProfile;
  const showLogout = authReady && authenticated;
  const showMobileMenu = showLogout;
  ...
  return (
    <header className="border-b border-[#1e2a3a] bg-[#12182080] backdrop-blur-sm">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-3 py-2 sm:gap-3 sm:px-6 sm:py-4">
        ...
        {showNav ? (
          <nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 text-sm md:flex md:gap-2">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
          {showLogout ? (
            <>
              {nickname ? (
                <span className="hidden max-w-[7rem] truncate rounded-lg border border-[#00d4aa33] bg-[#00d4aa1a] px-3 py-2 text-xs font-medium text-[#00d4aa] sm:inline-block">
                  {nickname}
                </span>
              ) : null}
              <button
                ...
                className="hidden rounded-lg border border-[#1e2a3a] px-3 py-2 text-xs text-[#8b9cb3] transition-colors hover:border-[#00d4aa4d] hover:text-[#e8edf4] md:inline-block"
              >
                로그아웃
              </button>
            </>
          ...
          {showNav && isAdmin ? (
            <Link
              href="/admin"
              className="hidden rounded-lg border border-[#00d4aa33] px-3 py-2 text-xs font-medium text-[#00d4aa] md:inline-block"
            >
              ADMIN
            </Link>
          ) : null}
          ...
          {showMobileMenu ? (
            <button
              ...
              className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[#1e2a3a] text-[#e8edf4] md:hidden"
            >
      ...
      {showMobileMenu && menuOpen ? (
        <div className="border-t border-[#1e2a3a] bg-[#121820] px-3 py-3 md:hidden">
          ...
          <div className={showNav ? "mt-3 space-y-1.5 border-t border-[#1e2a3a] pt-3" : "space-y-1.5"}>
            {nickname ? (
            ...
            {showLogout ? (
              <button
                type="button"
                onClick={() => void handleLogout()}
                className="w-full rounded-lg border border-[#1e2a3a] px-3 py-2 text-sm text-[#8b9cb3] transition-colors hover:text-[#e8edf4]"
              >
                로그아웃
              </button>
            ) : null}
